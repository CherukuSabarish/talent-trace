...
async function searchTalentData() {
  ...
  try {
    ...
    // Dedupe key: prefer the profile URL, but Crustdata doesn't always return one
    // (schema drift, or the person just has no linked profile field) - falling back
    // to url-only here used to silently drop every such candidate from the results
    // list even though the search legitimately found them (the "found" count and the
    // rendered list would disagree, like the built-for-Akamai example: "2 found" but
    // 0 candidates shown). personId, then name+company, keep them from vanishing.
    const cdKey = p => p.url || (p.personId ? 'pid:' + p.personId : '') ||
      'nc:' + (p.name || '').toLowerCase() + '|' + (p.company || '').toLowerCase();
    const existingKeys = new Set(cdResults.map(cdKey));
    const newOnes = found.filter(p => !existingKeys.has(cdKey(p)));
...
function talentDataPersonToProfile(person) {
  ...
  return {
    ...
    url,
    // Used as a dedupe fallback when Crustdata doesn't return a resolvable profile
    // URL for this person (see searchTalentData) - without this, such candidates
    // would be found by the search but silently excluded from the results list.
    personId: person.crustdata_person_id || '',
    snippet: bp.headline || '',
